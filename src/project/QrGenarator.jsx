...
const QR_TYPES = [
  { value: "text", label: "Plain Text", icon: "📄", placeholder: "Enter any text or message" },
  { value: "url", label: "Website URL", icon: "🌐", placeholder: "https://example.com" },
  { value: "phone", label: "Phone Number", icon: "📞", placeholder: "01700000000" },
  { value: "whatsapp", label: "WhatsApp", icon: "💬", placeholder: "01700000000" },
  { value: "email", label: "Email Address", icon: "✉️", placeholder: "you@example.com" },
  { value: "sms", label: "SMS Message", icon: "📱", placeholder: "01700000000" },
  { value: "vcard", label: "Contact (vCard)", icon: "👤", placeholder: "John Doe" },
  { value: "telegram", label: "Telegram", icon: "✈️", placeholder: "@username or username" },
  { value: "paypal", label: "PayPal", icon: "💳", placeholder: "yourpaypalusername" },
  { value: "bitcoin", label: "Bitcoin", icon: "₿", placeholder: "bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh" },
  { value: "wifi", label: "WiFi Network", icon: "📶", placeholder: "MyNetwork|password123" },
  { value: "location", label: "Location", icon: "📍", placeholder: "23.8103,90.4125" },
  { value: "event", label: "Calendar Event", icon: "📅", placeholder: "20251225T180000" },
];
...
function getDefaultQRSize() {
  return window.innerWidth < 768 ? 200 : 320; // Mobile: 200px, Desktop: 320px
}
...
export default function ProfessionalQRGenerator() {
  ...
  const darkMode = useSystemTheme();
  const currentQrType = QR_TYPES.find((t) => t.value === qrType);
  ...
  // Responsive default size on window resize
  useEffect(() => {
    const handleResize = () => {
      const newSize = getDefaultQRSize();
      if ([200, 320].includes(size)) {
        setSize(newSize);
      }
    };
  ...
  const presets = [
    { name: "Modern", fg: "#3b82f6", bg: "#ffffff", style: "rounded", corner: "extra-rounded" },
    { name: "Elegant Dark", fg: "#e2e8f0", bg: "#0f172a", style: "dots", corner: "rounded" },
    { name: "Vibrant", fg: "#7c3aed", bg: "#fefce8", style: "extra-rounded", corner: "dot" },
    { name: "Minimal", fg: "#111111", bg: "#ffffff", style: "square", corner: "square" },
  ];

  const mapPixel = useCallback((key) => {
    const map = { diamond: "dots", hex: "extra-rounded" };
    return map[key] || key;
  }, []);
  ...
  const formatData = useCallback(
    (text) => {
      ...
      switch (qrType) {
        ...
        case "sms":
          return `sms:${withCountry(trimmed)}`;

        case "telegram":
      // FIXED: Use deep link for direct app open
          const cleanUsername = trimmed.startsWith("@") ? trimmed.slice(1) : trimmed;
          return `tg://resolve?domain=${cleanUsername}`;

        case "vcard":
          return `BEGIN:VCARD\nVERSION:3.0\nFN:${trimmed}\nTEL:${withCountry(trimmed)}\nEND:VCARD`;
        ...
        case "wifi":
          const [ssid, pass = ""] = trimmed.split("|").map((s) => s.trim());
          const encryption = pass ? "WPA" : "none";
  ...
  useEffect(() => {
    qrInstance.current = new QRCodeStyling({
      ...
      cornersDotOptions: { type: "dot", color: fgColor },
      backgroundOptions: { color: bgColor },
      imageOptions: { crossOrigin: "anonymous", margin: 10, imageSize: logoSize / 100 },
    });
  ...
  }, [inputValue, size, margin, fgColor, bgColor, pixelStyle, cornerStyle, logo, logoSize, errorCorrection, qrType, countryCode]);
  ...
  const downloadQR = async (format = "svg") => {
    ...
    try {
      ...
      alert("Download failed. Try another format.");
    }
  };

  const handleWifiSSIDChange = (ssid) => {
    const password = inputValue.includes("|") ? inputValue.split("|")[1] : "";
    setInputValue(`${ssid}|${password}`);
  };

  const handleWifiPasswordChange = (password) => {
    const ssid = inputValue.split("|")[0] || "";
    setInputValue(`${ssid}|${password}`);
  };

  return (
    <div className={`min-h-screen py-8 px-4 sm:px-6 transition-colors duration-700 ${
      darkMode
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="inline-block p-1 rounded-3xl bg-gradient-to-r from-blue-500 to-purple-600">
            <div className={`rounded-3xl px-8 py-6 ${darkMode ? "bg-slate-900/95" : "bg-white/95"} backdrop-blur-xl`}>
              <h1 className="text-4xl sm:text-6xl font-black bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                QR Studio Pro
              </h1>
              <p className={`mt-2 text-base sm:text-lg ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
                Professional QR Code Generator
              </p>
            </div>
          </div>
        ...
        {/* Vertical Layout: Customize (Top) → Preview (Bottom) */}
        <div className="space-y-10">
          {/* Customize Panel */}
          <div className={`rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl border ${
            darkMode ? "bg-slate-800/70 border-slate-700/50" : "bg-white/85 border-gray-200/50"
          }`}>
            <h2 className="text-2xl sm:text-3xl font-bold mb-8 flex items-center gap-3">
              <span className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center text-white text-xl">🎨</span>
              Customize Your QR Code
            </h2>

            {/* QR Type */}
            <div className="mb-8">
              <label className="block text-sm font-semibold uppercase tracking-wider mb-4 text-gray-600 dark:text-gray-300">
                QR Type
              </label>
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                {QR_TYPES.map((type) => (
                  <button
                    key={type.value}
                    onClick={() => setQrType(type.value)}
                    className={`p-4 rounded-2xl text-xs sm:text-sm font-medium transition-all flex flex-col items-center gap-2 cursor-pointer ${
                      qrType === type.value
                        ? "bg-gradient-to-br from-blue-500 to-purple-600 text-white shadow-xl scale-105"
                        : darkMode
                        ? "bg-slate-700/70 hover:bg-slate-600 text-gray-100"
                        : "bg-gray-100/90 hover:bg-gray-200 text-gray-800"
                    }`}
                  >
            ...
            <div className="space-y-5 mb-8">
              <div className="flex items-center justify-between">
                ...
                {currentQrType && (
                  <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded-full text-xs font-medium flex items-center gap-1">
                    {currentQrType.icon} {currentQrType.label}
                  </span>
                )}
              </div>

              {(qrType === "phone" || qrType === "whatsapp" || qrType === "sms") && (
                <select
                  value={countryCode}
                  onChange={(e) => setCountryCode(e.target.value)}
                  className={`w-full p-4 rounded-2xl border-2 text-base ${
                    darkMode ? "bg-slate-700 border-slate-600 text-gray-100" : "bg-gray-50 border-gray-300 text-gray-800"
                  }`}
                >
                  {COUNTRY_CODES.map((c) => (
                    <option key={c.code} value={c.code}>
              ...
              {qrType === "wifi" && (
                <div className="space-y-4 p-5 rounded-2xl bg-amber-50/70 dark:bg-amber-900/30 border border-amber-200/50 dark:border-amber-800/50">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="MyWiFi"
                      className="w-full px-4 py-3 rounded-xl bg-white/70 dark:bg-slate-800 border border-gray-300 dark:border-slate-600"
                    />
                  </div>
                  <div>
              ...
              {qrType !== "wifi" && (
                <textarea
                  ...
                  className={`w-full p-5 rounded-2xl resize-none font-mono text-base border-2 transition-all focus:ring-4 focus:ring-purple-500/20 ${
                    darkMode
                      ? "bg-slate-800/70 border-slate-700 text-gray-100 placeholder-gray-500"
            ...
            {/* Presets */}
            <div className="mb-8">
              <label className="block text-sm font-semibold uppercase tracking-wider mb-4">Quick Styles</label>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {presets.map((p) => (
                  <button
                    key={p.name}
                    ...
                    className={`py-3 px-4 rounded-xl font-medium text-sm cursor-pointer ${
            ...
            <div className="mb-8">
              ...
              <input
                ...
                value={size}
                onChange={(e) => setSize(+e.target.value)}
                className="w-full h-3 rounded-full accent-purple-600 cursor-pointer"
              />
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <select
                  value={errorCorrection}
                  onChange={(e) => setErrorCorrection(e.target.value)}
                  className={`w-full p-3 rounded-xl ${darkMode ? "bg-slate-700" : "bg-gray-100"}`}
                >
              ...
              <div>
                <label className="block text-sm font-medium mb-3">Add Logo (Optional)</label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={handleLogoChange}
                  className={`w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-medium cursor-pointer ${
                    darkMode ? "file:bg-purple-600 file:text-white" : "file:bg-blue-600 file:text-white"
                  }`}
                />
                {logo && (
                  <div className="mt-4 flex items-center gap-4">
                    <img src={logo} alt="Logo" className="w-16 h-16 object-contain rounded-xl border" />
                    ...
                    <button onClick={() => setLogo(null)} className="text-red-500 hover:text-red-600 cursor-pointer">
                      Remove
                    </button>
                  </div>
                )}
          ...
          <div className={`rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl border ${
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl sm:text-3xl font-bold flex items-center gap-3">
                <span className="w-10 h-10 bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl flex items-center justify-center text-white text-xl">👁️</span>
                Live Preview
              </h2>
            ...
            <div className={`flex items-center justify-center p-10 rounded-3xl ${
              darkMode ? "bg-slate-900/60" : "bg-gray-50/70"
            } ${animatePreview ? "animate-float" : ""}`}>
              <div ref={qrRef} className="shadow-2xl rounded-3xl overflow-hidden border-8 border-white dark:border-slate-800" />
            </div>

            <div className="mt-10 grid grid-cols-3 gap-4">
              <button onClick={() => downloadQR("svg")} className="py-4 rounded-2xl font-bold text-white bg-gradient-to-r from-blue-600 to-purple-600 hover:scale-105 transition-transform cursor-pointer">
                SVG
              </button>
              <button onClick={() => downloadQR("png")} className="py-4 rounded-2xl font-bold text-white bg-gradient-to-r from-emerald-500 to-teal-600 hover:scale-105 transition-transform cursor-pointer">
                PNG
              </button>
              <button onClick={() => downloadQR("jpg")} className="py-4 rounded-2xl font-bold text-white bg-gradient-to-r from-orange-500 to-red-600 hover:scale-105 transition-transform cursor-pointer">
                JPG
              </button>
            </div>
          </div>
