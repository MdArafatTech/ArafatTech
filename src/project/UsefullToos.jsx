    ...
    <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 md:p-8 shadow-xl border border-slate-200 dark:border-slate-700">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-8 pl-1 pr-5 bg-gradient-to-br from-slate-50/50 to-slate-100/50 dark:from-slate-900/50 dark:to-slate-800/50 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-300">
        {[
          ...
          {
            label: "Brackets",
            state: includeBrackets,
            set: setIncludeBrackets,
            icon: "[]{}",
          },
          {
            label: "Ambiguous",
            state: includeAmbiguous,
            set: setIncludeAmbiguous,
            icon: "0OIl",
          },
          {
            label: "Emojis",
            state: includeEmojis,
            set: setIncludeEmojis,
            icon: "😀",
          },
          {
            label: "Hyphens",
            state: includeHyphens,
            set: setIncludeHyphens,
            icon: "-",
          },
        ...
        <div className="relative" aria-hidden="true">
          ...
          <div
            className={`absolute top-1 w-6 h-6 bg-white rounded-full shadow-lg transition-all duration-300 ease-in-out ${
              passphraseMode ? "translate-x-6 shadow-xl " : "left-1"
            }`}
          />
  ...
  useEffect(() => {
    const fetchCurrencies = async () => {
      ...
      try {
        ...
        const rates = Object.keys(res.data.rates).filter(
          ...
          <div className="w-full relative flex flex-col items-center justify-center">
            <div className="text-2xl md:text-3xl lg:text-4xl font-bold text-emerald-400 break-words leading-tight max-w-full px-2">
              ...
              <span className="text-3xl">
                {currencyData[toCurrency]?.symbol || toCurrency}
              </span>
            </div>
          </div>
        ) : (
...
const UsefulTools = () => {
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 via-white to-amber-50 dark:from-gray-900 dark:via-gray-800 dark:to-slate-900 p-4">
        <LoginModal 
          onClose={() => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto">
            Professional password generator & currency converter — fast,
            private, beautiful
          </p>
        </div>
